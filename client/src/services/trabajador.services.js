...
export const addTrabajador = async (values) =>
  await axios.post("http://localhost:8000/api/crear-trabajador", values);

export const getAllTrabajadores = async () =>
  await axios.get("http://localhost:8000/api/get-all-trabajadores");

export const getAllTrabajadoresOfAJornada = async (date) =>
  await axios.get(`http://localhost:8000/api/obtener-jornada/${date}`);

export const deleteOneTrabajador = async (rut) =>
  await axios.delete(`http://localhost:8000/api/delete-trabajador/${rut}`);

export const updateTrabajador = async (values, rut) =>
  await axios.put(
    `http://localhost:8000/api/actualizar-datos-trabajador/${rut}`,
    values
  );

export const getOneTrabajador = async (rut) =>
  await axios.get(`http://localhost:8000/api/obtener-trabajador/${rut}`);
...
export const createLicencia = async (rut, values) =>
  await axios.put(
    `http://localhost:8000/api/actualizar-licencia/${rut}`,
    values
  );

export const getTrabajadoresWithLicencia = async () =>
  await axios.get("http://localhost:8000/api/obtener-trabajadores-licencia");

export const getInformeVisualMes = async (values) =>
  await axios.post(
    "http://localhost:8000/api/obtener-informe-visual-mes",
    values
  );

export const resetLicencia = async (values) =>
  await axios.post("http://localhost:8000/api/reset-licencia", values);
